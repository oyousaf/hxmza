...
export type CarModel = {
  ...
  yearFrom?: number;
  yearTo?: number | null;
  [key: string]: unknown; // allows safe extensibility
};
...
export async function fetchModels(makeId: number): Promise<CarModel[]> {
  ...
  const models: CarModel[] = Array.isArray(res)
    ? res.map((m, idx) => ({
        ...
        yearFrom: (m as any).yearFrom ? Number((m as any).yearFrom) : undefined,
        yearTo: (m as any).yearTo ? Number((m as any).yearTo) : null,
